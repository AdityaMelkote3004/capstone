...
def run_pipeline_inprocess(csv_path: str, out_dir: str):
    """
    Run the entire baseline pipeline by importing and calling functions directly.
    No subprocess -- works on Windows without encoding issues.
    ...
    try:
        with contextlib.redirect_stdout(buf):
            ...
            plot_all(results, df, output_dir=out_dir)
            save_all(results, output_dir=out_dir)
...
if run_btn:
    ...
    prog.progress(15, text="Loading and preprocessing data...")

    success, log = run_pipeline_inprocess(csv_path, str(RESULTS_DIR))

    prog.progress(100, text="Done!" if success else "Failed.")

    if success:
        st.success("Pipeline complete! Refresh the page or navigate to Overview to see results.")
        st.cache_data.clear()
    else:
...
if not results_exist():
    st.warning("No results yet. Set the CSV path in the sidebar and click **Run Baseline**.")
    st.stop()
...
if page == "Overview":
    ...
    for col, step, colour in zip(cols, steps, STEP_COLOURS):
        with col:
            label = step["name"].split("--")[1].strip() if "--" in step["name"] else step["name"]
            st.markdown(
    ...
    # Grouped bar chart
    st.subheader("Metrics Across All Steps")
    ...
    for col, (label, delta) in zip(d_cols, deltas):
        col.metric(label, f"{delta:+.4f} AUC",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_stats:
        ...
        st.markdown(f"- Specificity (TNR): `{tn/(tn+fp):.3f}`")
        st.markdown(f"- Recall (TPR):      `{tp/(tp+fn):.3f}`")
        st.markdown(f"- Precision:         `{tp/(tp+fp):.3f}`")
        st.markdown(f"- Total test rows:   `{total}`")
        st.markdown(f"- Correct:           `{tn+tp}` / `{total}`")
    ...
    desc = {
        0: "**15 lagged technical indicators** -- RSI, MACD, MACD_Signal, MACD_Hist, "
           ...
           "All shifted 1 trading day per ticker to prevent target leakage.",
        1: "Price features **+** 3 tweet-count columns -- "
           "`Company_Tweet_Count`, `Event_Tweet_Count`, `Total_Tweet_Count` (also lagged 1 day). "
           "No NLP -- just volume of social media activity as a signal proxy.",
        2: "Price features **+** all 14 EDGAR fundamentals -- Revenue, NetIncome, EPS, "
        ...
        3: "**All modalities combined** -- Price (15) + Sentiment (3) + Fundamentals (28) = 46 features. "
           "Upper bound for any non-deep-learning model on this data.",
    }
    st.info(desc[idx])
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    sector_frames = []
    for slug, sname in zip(STEP_SLUGS, ["S1","S2","S3","S4"]):
    ...
    if sector_frames:
        ...
        st.subheader("AUC Heatmap -- Sector x Step")
        ...
        st.markdown("---")
        st.subheader("AUC Trend by Sector")
        selected_sectors = st.multiselect(
            "Select sectors", options=sectors, default=sectors[:4]
        )
        fig_line = go.Figure()
        ...
        for _, row in heatmap_df.iterrows():
            ...
            label = {"S1": "Price Only", "S2": "+Sentiment",
                     "S3": "+Fundamentals", "S4": "Full"}[best]
            best_rows.append({"Sector": row["Sector"], "Best Step": label,
                              "Best AUC": round(vals[best], 4)})
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption("LR coefficients from the Full Structured model (Step 4)")
    st.markdown("---")
    st.info(
        "All features are z-score normalised, so coefficients are directly comparable. "
        "Positive = pushes toward UP (1). Negative = pushes toward DOWN (0)."
    )

    if st.button("Load Feature Coefficients"):
        with st.spinner("Training model and extracting coefficients..."):
            try:
                ...
                st.markdown("**Red** = pushes toward UP &nbsp;|&nbsp; **Blue** = pushes toward DOWN")

            except Exception as e:
                st.error(f"Error: {e}")
                st.code(traceback.format_exc())
    else:
        st.markdown("Click the button above to extract and plot feature coefficients.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if "df" in st.session_state:
        ...
        c4.metric("Date Range", f"{df['Date'].min().date()} to {df['Date'].max().date()}")
        ...
        fig_bal = px.bar(
            ...
            color="Pct_Up", color_continuous_scale="RdYlGn",
            range_color=[45, 55],
            labels={"Pct_Up": "% Days Up"},
            template="plotly_dark",
        )
        fig_bal.add_vline(x=50, line_dash="dash", line_color="white",
                          annotation_text="50% balanced")
        fig_bal.update_layout(
            paper_bgcolor="#000000", plot_bgcolor="#1C1C1E",
            height=360, coloraxis_showscale=False, margin=dict(t=10),
        )
